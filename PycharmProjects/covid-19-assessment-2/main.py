...
def run():
    ...
    try:
        with open("data/covid_19_dataset.csv") as file:
            data = csv.reader(file)
            for row in data:
                covid_records.append(row)
            print(covid_records)
        for row in covid_records[1:]:
            progress("Loading data", int(row[0]))
    except IOError:
        error("File cannot be found or loaded.")
    ...
    while True:
        ...
        # TODO: Your code here
        if user_option == 1:
            progress("Data processing", 0)
            sub_menu_opt = menu(1)
            if sub_menu_opt == 1:
                progress("Record retrieve", 0)
                print(retrieve_record_by_serial_no(covid_records))
                progress("Record retrieve", 100)
            elif sub_menu_opt == 2:
                progress("Records retrieve", 0)
                records_by_selected_dates = retrieve_records_by_date(covid_records)
                display_records(records_by_selected_dates)
                progress("Records retrieve", 100)
            elif sub_menu_opt == 3:
                progress("Grouping process", 0)
                retrieve_records_by_region(covid_records)
                print("A function from tui.py")
                progress("Grouping process", 100)
            elif sub_menu_opt == 4:
                progress("Summary process", 0)
                print(retrieve_summary(covid_records))
        ...
        # TODO: Your code here
            ...
            progress("Visualisation process", 0)
            sub_menu_opt = menu(2)
            if sub_menu_opt == 1:
                display_pie_chart(covid_records)
            elif sub_menu_opt == 2:
                display_top_five_countries(covid_records)
            elif sub_menu_opt == 3:
                display_animation()
            progress("Visualisation process", 100)
